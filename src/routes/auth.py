...
async def request_email(body: RequestEmail, background_tasks: BackgroundTasks, request: Request,
    ...
    email address, and then checks if there is already a user with that email in the database. If there isn't,
    then it creates one and sends them an email confirmation link. If there is already a user with that 
    email in the database but they haven't confirmed their account yet (i.e., confirmed = False), then it 
    sends them another confirmation link.
    
    :param body: RequestEmail: Get the email from the request body
    :param background_tasks: BackgroundTasks: Add a task to the background tasks queue
    :param request: Request: Get the base url of the application
    :param db: Session: Get the database session
    :return: A message to the user, but it does not return a user object
    :doc-author: Trelent
    """
